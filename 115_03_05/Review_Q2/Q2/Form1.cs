...
namespace Q2
{
    public partial class Form1 : Form
    {
        ...
        {
            // Initialize state for new layout
        ...
        {
            // safe parse parts and hours from textboxes
        ...
        private ServiceRecord CreateRecordFromInputs()
        {
            var selected = GetSelectedServiceItems().ToList();
            decimal serviceFee = selected.Sum(s => s.Price);
            var displayNames = selected.Select(s => s.DisplayName).ToList();

            if (!decimal.TryParse(textBoxPartsInput.Text.Trim(), out var parts)) parts = 0m;
            if (!decimal.TryParse(textBoxHoursInput.Text.Trim(), out var hours)) hours = 0m;
        ...
        // Keep export/report/save methods intact (they use records list)
